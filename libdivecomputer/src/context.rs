...
impl Context {
    pub fn set_loglevel(&mut self, loglevel: LogLevel) -> Result<()> {
        let status =
            unsafe { ffi::dc_context_set_loglevel(self.ptr, loglevel as ffi::dc_loglevel_t) };
...
// Safe wrapper ensures context can be sent between threads unless the C library supports it
