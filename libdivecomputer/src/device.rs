...
use crate::{
    Context, DiveComputer, c_void_as,
    common::Status,
...
pub struct Device<'ctx, 'item, T: DeviceState> {
    ...
    pub(crate) item: &'item DescriptorItem<'ctx>,
...
impl<'ctx, 'item> Device<'ctx, 'item, DeviceDisconnected> {
    ...
    pub fn connect(mut self) -> Result<Device<'ctx, 'item, DeviceConnected>> {
        self.data.transport = self.transport.clone();
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match &self {
            ...
            Self::UsbHid {
                ...
                write!(f, "USB HID: {vendor_id:04X}:{product_id:04X}")
            }
            Self::Bluetooth {
                name,
                address_string,
            ...
            Self::Irda { name, address } => {
                write!(f, "IrDA: {name} (0x{address:08X})")
            }
        }
    }
}
