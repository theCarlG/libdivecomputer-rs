...
use crate::device::{DeviceData, DeviceTransport};
use crate::{c_void_as, error};
...
impl BleTransport {
    pub fn scan(duration: Duration) -> error::Result<Vec<DeviceTransport>> {
        ...
        let rt = match tokio_runtime() {
            ...
            Err(err) => {
                ...
                return Err(error::LibError::status(ffi::DC_STATUS_IO));
            }
        };
        ...
        .map_err(crate::error::LibError::Btleplug)
    }
    ...
    pub async fn connect(
        ...
        let peripheral = if let Some(p) = peripheral {
            p
        } else {
            let manager = Manager::new().await?;
            let adapters = manager.adapters().await?;
            ...
            Self::find_peripheral(&adapter, mac_address).await?
        };
...
#[cfg(target_os = "android")]
std::thread_local! {
  static JNI_ENV: std::cell::RefCell<Option<AttachGuard<'static>>> = std::cell::RefCell::new(None);
}
    ...
    let runtime = {
        tokio::runtime::Builder::new_multi_thread()
            ...
            .on_thread_start(move || {
                ...
                {
                    eprintln!("new thread, restoring env");
...
extern "C" fn ble_open(
    ...
    let data = unsafe { c_void_as!(userdata, DeviceData) };
    let peripheral = if let DeviceTransport::Ble { peripheral, .. } = &data.transport {
        Some(peripheral.clone())
    } else {
        None
    };
    let transport = match rt.block_on(BleTransport::connect(addr, peripheral)) {
        Ok(t) => Box::new(t),
        Err(err) => {
